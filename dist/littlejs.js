...
function debugRender() {
  ...
  if (debugOverlay) {
    ...
    for (const o of engineObjects) {
      ...
      const color2 = o.parent
        ? new Color(1, 1, 1, 0.5)
        : new Color(0, 0, 0, 0.8);
      drawRect(o.pos, size, color1, o.angle, false);
      drawRect(o.pos, size.scale(0.8), color2, o.angle, false);
      o.parent &&
        drawLine(o.pos, o.parent.pos, 0.1, new Color(0, 0, 1, 0.5), false);
    }
    ...
    if (bestObject) {
      ...
      drawLine(
        ...
        bestObject.pos,
        0.1,
        raycastHitPos ? new Color(1, 0, 0, 0.5) : new Color(0, 1, 0, 0.5),
        false
      );
  ...
  update() {
    ...
    if (parent) {
      ...
      this.pos = this.localPos
        .multiply(vec2(parent.getMirrorSign(), 1))
        .rotate(-parent.angle)
        .add(parent.pos);
    ...
    if (this.collideSolidObjects) {
      ...
      for (const o of engineObjectsCollide) {
        ...
        if (isOverlapping(oldPos, this.size, o.pos, o.size)) {
          ...
          debugOverlay &&
            debugPhysics &&
            debugAABB(this.pos, this.size, o.pos, o.size, "#f00");
          continue;
        }
        ...
        debugOverlay &&
          debugPhysics &&
          debugAABB(this.pos, this.size, o.pos, o.size, "#f0f");
      }
    }
      ...
      if (tileCollisionTest(this.pos, this.size, this)) {
        ...
        if (!tileCollisionTest(oldPos, this.size, this)) {
          ...
          if (isBlockedY || !isBlockedX) {
            ...
            const o =
              ((oldPos.y - this.size.y / 2) | 0) - (oldPos.y - this.size.y / 2);
            if (
              o < 0 &&
              o > this.damping * this.velocity.y + gravity * this.gravityScale
            )
              this.velocity.y = this.damping
                ? (o - gravity * this.gravityScale) / this.damping
                : 0;
...
function mouseToScreen(mousePos) {
  ...
  return vec2(mainCanvas.width, mainCanvas.height).multiply(
    vec2(
      percent(mousePos.x, rect.left, rect.right),
      percent(mousePos.y, rect.top, rect.bottom)
    )
  );
}
...
function gamepadsUpdate() {
  ...
  if (touchGamepadEnable && isTouchDevice) {
    ...
    if (!touchGamepadButtons) createTouchGamepad();
...
function touchGamepadRender() {
  ...
  overlayContext.fillStyle =
    touchGamepadStick.lengthSquared() > 0 ? "#fff" : "#000";
  overlayContext.beginPath();
...
function medalsInit(saveName) {
  ...
  debugMedals ||
    medals.forEach(
      (medal) => (medal.unlocked = localStorage[medal.storageKey()] | 0)
    );
}
...
class Medal {
  ...
  render(hidePercent = 0) {
    ...
    this.renderIcon(
      vec2(x + 15 + medalDisplayIconSize / 2, y + medalDisplaySize.y / 2)
    );
...
class Newgrounds {
  ...
  constructor(app_id, cipher, cryptoJS) {
    ASSERT(
      !newgrounds && app_id > 0,
      "there can only be one newgrounds object"
    );
...
function glPreRender() {
  ...
  glContext.uniformMatrix4fv(
    ...
    false,
    new Float32Array([s.x, 0, 0, 0, 0, s.y, 0, 0, 1, 1, 1, 1, p.x, p.y, 0, 0])
  );
}
...
function glInitPostProcess(shaderCode, includeOverlay = false) {
  ...
  if (glPostIncludeOverlay) overlayCanvas.style.visibility = "hidden";
}
...
function glRenderPostProcess() {
  ...
  const uniformLocation = (name) =>
    glContext.getUniformLocation(glPostShader, name);
